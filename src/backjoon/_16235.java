    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<k; i++){
            ...
            for(int x=0; x<n; x++){
                for(int y=0; y<n; y++){
                    // 나이어린 순으로 양분 주기
                    if(lst.get(x).get(y).isEmpty()) continue;

                    int pQSize = lst.get(x).get(y).size();
                    // TODO : 오늘 해야할 것.
                    for(int idx = 0; idx < pQSize; idx ++) {
                        ...
                        if (treeAge > nutrient) {
                            ...
                            nutrientsArr[x][y] = (int) Math.floor(treeAge / 5.0);
                            continue;
                        }
                        nutrientsArr[x][y] -= treeAge;
                        lst.get(x).get(y).add((treeAge + 1));
                    }
                }
            }
...
//                for (List<PriorityQueue<Integer>> priorityQueues : lst) {
//                    for (PriorityQueue<Integer> priorityQueue : priorityQueues) {
